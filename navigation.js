	...
	async function fetchMenu(menuSlug, menuId = null) {
		try {
			...
			let response = await fetch(
				`${WP_CUSTOM_API}/menus/${locationSlug}`
			);
			...
			if (alternateLocations[locationSlug]) {
				response = await fetch(
					`${WP_CUSTOM_API}/menus/${alternateLocations[locationSlug]}`
				);
				if (response.ok) {
					const menuItems = await response.json();
					return menuItems;
			...
			if (menusResponse.ok) {
				...
				const menu = menus.find(
					(m) => m.slug === locationSlug || m.name === menuSlug
				);
	...
	function initMobileMenuToggles() {
		...
		submenuToggles.forEach((toggle) => {
			toggle.addEventListener("click", function (e) {
				...
				if (submenu) {
					const isActive =
						parentLi.classList.contains("submenu-active");
					...
					if (isActive) {
						...
						requestAnimationFrame(() => {
							submenu.style.transition =
								"height 300ms ease, padding 300ms ease";
